...
// ============================================================================
// Community-maintained modular Bicep deployment for FinOps Hub.
// Based on Microsoft FinOps Toolkit patterns, with enterprise customization support.
// NOT officially supported by Microsoft. See: https://github.com/microsoft/finops-toolkit
...
var effectiveAdxCapacity = dataExplorerCapacity > 0
  ? dataExplorerCapacity
  : (startsWith(effectiveAdxSku, 'Dev') ? 1 : 2)
...
var adxClusterName = !empty(dataExplorerClusterName)
  ? take(toLower('${replace(dataExplorerClusterName, '-', '')}${take(uniqueSuffix, 6)}'), 22)
  : ''
...
var effectiveIdentityPrincipalId = useExistingIdentity
  ? existingManagedIdentity!.properties.principalId
  : managedIdentity!.outputs.principalId
var effectiveIdentityClientId = useExistingIdentity
  ? existingManagedIdentity!.properties.clientId
  : managedIdentity!.outputs.clientId
var effectiveIdentityResourceId = useExistingIdentity
...
output adxSchemaComponents array = deployAdxSchema && adxSchemaSetup != null
  ? adxSchemaSetup!.outputs.deployedComponents
  : []
...
output exportConfiguration object = {
  ...
  // Demo mode commands - test data generator available from the official FinOps Toolkit
  demoCommands: {
    installToolkit: 'Install-Module FinOpsToolkit -Force'
    testDataDocs: 'https://aka.ms/finops/toolkit'
    note: 'Test data generation scripts are maintained in the official Microsoft FinOps Toolkit repository: https://github.com/microsoft/finops-toolkit'
  }
}

// Use variables to build arrays that can then be used in outputs
var settingsJsonScopes = [
  for scope in scopesToMonitor: {
    scope: scope.?scopeId ?? ''
...
output gettingStartedGuide object = {
  ...
  demoModeSteps: [
    ...
    '2. Generate test data using the official FinOps Toolkit scripts: https://aka.ms/finops/toolkit'
    '3. Upload test data to the msexports container in storage account: ${storageAccount.outputs.name}'
    '4. Wait ~5 minutes for ADF pipelines to process the data'
  ...
  commonSteps: [
    ...
    'Get official Power BI reports and ADX dashboards from the FinOps Toolkit: https://aka.ms/finops/toolkit'
