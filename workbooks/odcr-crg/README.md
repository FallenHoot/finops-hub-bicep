...
FinOps hub is an enrichment layer, not a workbook dependency. Without it, the workbook still shows the reservation estate, ownership, sharing authorization, compute references, and authoritative current allocation for a selected CRG. With FinOps hub enabled, the workbook adds historical `EffectiveCost` for explicit `Used` and `Unused` ODCR billing rows.
...
6. Leave **Use FinOps hub billing data** as **No** for standalone Azure Resource Graph mode.
...
|---|---|---|
| Operations mode | Azure Resource Graph plus ARM Instance View | Provider subscription, reservation inventory, sharing profile, compute references, and current allocation for a selected CRG. |
| FinOps hub enhanced mode | Operations mode plus FinOps hub ADX | All operational views plus actual unused ODCR cost, billing trend, and billing-backed CRG and resource rankings. |
...
| Tab | What it shows |
|---|---|
| Overview | Estate counts from ARG and optional financial impact from FinOps hub. |
...
| Cost | Optional FinOps hub cost summary, trend, CRG ranking, and resource detail. |
...
| If FinOps exists, how much has unused capacity actually cost us? | Unused `EffectiveCost` summary, trend, CRG ranking, and reservation detail by billing currency. | FinOps hub ADX | Complete for selected billing window and available exports |
...
|---|---|---|
| Actual unused ODCR cost | FinOps hub ADX billing data | Financial impact, reporting, trend confirmation. |
